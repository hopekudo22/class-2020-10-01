...
#five separate data frames for each year, adjusting skip since it changes each year
...
#binded rows to create an id column called year
classes <- bind_rows(d_2015, d_2016, d_2017, d_2018, d_2019, .id = "year") %>%
  #create tidy format
  mutate(year = as.numeric(year) + 2014) %>%
#see dynamic of how enrollment of classes change
pivot_wider(names_from = year, values_from = u_grad, names_prefix = "ugrad_")
...
big_classes <- function(x, my_year = ugrad_2019) {
  stopifnot
  high_tibble <- classes %>%
    #greatest to least from value of year passed
    arrange(desc(ugrad_2019)) %>%
    slice_head(n = 5)
